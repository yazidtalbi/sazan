...
export function mountMasterplan({host,markers,zoomLabel,zone,locations,getSelected,onReady,onError,onQuality,reduced}) {
  ...
  let width=1,height=1,data,terrain,baseMaterial,shadowSurface,waterMaterial,overviewTexture,highTexture,renderer,clouds;
  ...
  const trackMaterial=m=>{materials.add(m);clouds?.applyTo(m);return m;};
  ...
  function projectedRoof(material) {
    ...
    m.customProgramCacheKey=()=> 'sazan-registered-roofs';trackMaterial(m);return m;
  }
  ...
  const visibilityChanged=()=>{last=0;};
  ...
  function animate(now){
    ...
    // The camera solver handles elapsed time exactly. Capping its delta would
    // turn a three-second dolly into a much longer crawl on slower devices.
    const dt=Math.max(0,(now-(last||now))/1000);last=now;elapsed+=Math.min(.05,dt);
    const zoom=rig.step(dt,motion&&!mobile);
    clouds?.update(dt,{motion,reduced,zoom});
    ...
    if(Math.floor(now/250)!==Math.floor((now-dt*1000)/250)){
      host.dataset.zoom=zoom.toFixed(3);host.dataset.angle=rig.state.angle.toFixed(2);host.dataset.fov=rig.state.fov.toFixed(2);
      host.dataset.drawCalls=String(renderer.info.render.calls);host.dataset.triangles=String(renderer.info.render.triangles);
      if(clouds){host.dataset.cloudTime=clouds.state.time.toFixed(3);host.dataset.cloudShadowX=clouds.state.shadowX.toFixed(3);host.dataset.cloudCount=String(clouds.state.count);}
    }
  }
  Promise.all([fetchFile(ASSETS+'registration.json','json'),texture(ASSETS+'map-4k.webp'),texture('/masterplan/island-water-mask.png',false)]).then(([registration,map,mask])=>{
    ...
    texture('/masterplan/cloud.webp').then(cloudTexture=>{
      if(disposed)return;
      clouds=createClouds(cloudTexture,{mobile});materials.forEach(material=>clouds.applyTo(material));scene.add(clouds.group);
    }).catch(()=>{}); // Optional atmosphere must never block the masterplan.
  ...
  return {
    ...
    dispose(){
      ...
      clouds?.dispose();directional.shadow.dispose();release();renderer.dispose();renderer.forceContextLoss();renderer.domElement.remove();
